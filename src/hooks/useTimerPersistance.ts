...
import { convertTimestampToMillis } from "../lib/firestore-helpers";

type TimestampLike = Timestamp | Date | number | null;
 ...
 */
export function useTimerPersistence(projectId: string | null | undefined, businessId: string | null | undefined) {
  const [timerSeconds, setTimerSeconds] = useState(0);
  const [timerStartedAt, setTimerStartedAt] = useState<TimestampLike>(null);
  const [timerPausedAt, setTimerPausedAt] = useState<TimestampLike>(null);
  ...
  useEffect(() => {
    async function loadTimerState() {
      try {
        ...
        const fetchedStartedAt: TimestampLike = projectData.timerStartedAt || null;
        const fetchedPausedAt: TimestampLike = projectData.timerPausedAt || null;
        const fetchedIsActive = projectData.isActive === true;

        // Convert Firestore Timestamps to milliseconds for consistent state storage
        const startedAtMs = convertTimestampToMillis(fetchedStartedAt);
        const pausedAtMs = convertTimestampToMillis(fetchedPausedAt);

        // Cache the millisecond values
        if (startedAtMs) {
          localStorage.setItem(CACHE_START_KEY, String(startedAtMs));
        }
        if (pausedAtMs) {
          localStorage.setItem(CACHE_PAUSE_KEY, String(pausedAtMs));
        }
        localStorage.setItem(CACHE_ACTIVE_KEY, String(fetchedIsActive));
  ...
  const calculateElapsedSeconds = useCallback(() => {
    ...
    const startMs = typeof timerStartedAt === 'number'
      ? timerStartedAt
      : ("toMillis" in timerStartedAt ? timerStartedAt.toMillis() : timerStartedAt.getTime());
    ...
    if (isActive) {
      ...
      const pauseMs = typeof timerPausedAt === 'number'
        ? timerPausedAt
        : ("toMillis" in timerPausedAt ? timerPausedAt.toMillis() : timerPausedAt.getTime());
  ...
  const startTimer = useCallback(async () => {
    ...
    localStorage.setItem(CACHE_START_KEY, String(nowMs));
  ...
  const pauseTimer = useCallback(async () => {
    ...
    localStorage.setItem(CACHE_PAUSE_KEY, String(nowMs));
  ...
  const resumeTimer = useCallback(async () => {
    ...
    const startMs = typeof timerStartedAt === 'number' ? timerStartedAt : 0;
    const pauseMs = typeof timerPausedAt === 'number' ? timerPausedAt : 0;

    if (isMountedRef.current) {
      // Time that was accumulated before pause
      const elapsedBeforePauseMs = pauseMs - startMs;
    ...
    localStorage.setItem(CACHE_START_KEY, String(adjustedStartTimeMs));
