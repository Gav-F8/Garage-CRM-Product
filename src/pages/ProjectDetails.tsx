...
import { useTimerPersistence } from "../hooks/useTimerPersistance";
import { statusStyle } from "../lib/utils";
import { NavigationBar } from "../components/NavigationBar";
import { notionClasses } from "../lib/notion-theme";
import { useAuth } from "../context/AuthContext";
import type { WithId, Project, Vehicle, Employee, TimeLog, ProjectNote } from "@/types/firestore";
...
export default function ProjectDetailsPage() {
  ...
  const [carDetails, setCarDetails] = useState<WithId<Vehicle> | null>(null);
  const [notes, setNotes] = useState<WithId<ProjectNote>[]>([]);
  const [timeLogs, setTimeLogs] = useState<WithId<TimeLog>[]>([]);
  ...
  const notesUnsubscribeRef = useRef<Unsubscribe | null>(null);
  const timeLogsUnsubscribeRef = useRef<Unsubscribe | null>(null);
  ...
  useEffect(() => {
    async function loadProjectData() {
      ...
      try {
        ...
        setCurrentEmployee(employeeSnap.data() as Employee);
        ...
        const projectData = { id: projectSnap.id, ...projectSnap.data() } as WithId<Project>;
        ...
        if (projectData.vehicleId) {
          ...
          if (carSnap.exists()) {
            setCarDetails({ id: carSnap.id, ...carSnap.data() } as WithId<Vehicle>);
          } else {
            setCarDetails(null);
          }
        } else {
  ...
  async function handleAddNote() {
    ...
    try {
      const success = await createProjectNotes(businessId as string, projectId as string, {
        text: trimmedNote,
  ...
  async function handleAddTimeLog() {
    ...
    try {
      const success = await createProjectTimelog(businessId as string, projectId as string, {
  ...
  function startEditingLog(log: WithId<TimeLog>) {
  ...
  async function handleUpdateTimeLog(logId: string) {
    ...
    const success = await updateProjectTimelogValue(businessId as string, projectId as string, logId, {
  ...
  function canManageLog(log: WithId<TimeLog>) {
    const currentUid = auth.currentUser?.uid;
                ...
                <div className="mt-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.assignedMechanicName?.length ? (
                      (Array.isArray(project.assignedMechanicName)
                  ...
                  {notes.length ? (
                    ...
                    >
                      <p className="text-sm text-[#37352F] mb-3 break-words whitespace-pre-wrap">
                        {(project as unknown as { description?: string }).description || "No description available"}
                      </p>
                      ...
                      >
                        ...
                        <div className="flex flex-col gap-1 text-xs text-[#787774]">
                          ...
                          <span>
                            ...
                            {formatTimestamp(note.createdAt as any)}
                          </span>
                        </div>
                      </div>
                    ))
