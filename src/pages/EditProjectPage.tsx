...
import { useCustomersForCurrentUser } from "../hooks/useCustomersForCurrentUser";
import { useAuth } from "../context/AuthContext";
import { fetchMechanics, fetchVehicles, fetchProjectDetail, updateProjectValue } from "../lib/firestore-helpers";
import { STATUS_OPTIONS } from "../lib/utils";
import { notionClasses } from "../lib/notion-theme";
import { NavigationBar } from "../components/NavigationBar";
import type { WithId, Vehicle } from "@/types/firestore";
...
export default function EditProjectPage() {
  ...
  useEffect(() => {
    ...
    async function loadProject() {
      ...
      try {
        const projectSnap = await fetchProjectDetail(businessId, projectId as string)
        if (!projectSnap) {
        ...
        const projectData = projectSnap as unknown as Record<string, unknown>;
        ...
        setForm({
          ...
          assignedMechanicIds: Array.isArray(projectData.assignedMechanicIds)
          ? projectData.assignedMechanicIds
          : Object.keys((projectData.assignedMechanicIds as object) || {}),
        });
      } finally {
        setLoading(false);
      }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const success = await updateProjectValue(businessId as string, projectId as string, form);
