...
export default function VehiclePage() {
  ...
  const customers = customersList.reduce<Record<string, string>>((acc, c) => {
    acc[c.id] = c.name || "";
    return acc;
  }, {});

  const [items, setItems] = useState<WithId<Vehicle>[]>([]);
      ...
      <div className={notionClasses.dashboardContainer}>
        ...
        {showModal && businessId && (
          <CreateVehicleModal
            ...
            onCreated={(newItem) => {
              setItems((p) => [newItem as WithId<Vehicle>, ...p]);
              setTotalHoursMap((p) => ({ ...p, [(newItem as { id: string }).id]: "0h 0m" }));
            }}
          />
        )}
      </div>
