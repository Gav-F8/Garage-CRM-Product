...
import type { WithId, Employee } from "@/types/firestore";
import {
  Loader2,
  UserCheck,
  UserX,
...
type EmployeeRecord = WithId<Employee>;
...
type ConfirmActionType = "approve" | "decline" | "remove";

// ── Confirm Modal ─────────────────────────────────────────────────────────────
function ConfirmModal({ action, emp, onConfirm, onCancel }: {
  action?: ConfirmActionType;
  emp?: EmployeeRecord | null;
  onConfirm: () => void;
  onCancel: () => void;
}) {
...
// ── Detail Modal ──────────────────────────────────────────────────────────────
function EmployeeDetailModal({ emp, onClose }: { emp: EmployeeRecord | null; onClose: () => void }) {
  if (!emp) return null;

  const joinDate = emp.createdAt
    ? new Date(emp.createdAt as string).toLocaleDateString(undefined, {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
  ...
  const roleBadge =
    ...
    } as Record<string, string>)[emp.role ?? ""] ?? "bg-gray-100 text-gray-700 border-gray-200";
  ...
  const roleLabel =
    ({
      ...
      rejected: "Rejected",
      owner: "Owner",
    } as Record<string, string>)[emp.role ?? ""] ?? emp.role;
...
function EditEmployeeModal({ emp, onSave, onClose, saving }: {
  emp: EmployeeRecord | null;
  onSave: (uid: string, status: string, role: string) => void;
  onClose: () => void;
  saving: boolean;
}) {
...
type EmployeeCardVariant = "pending" | "active" | "suspended" | "rejected";
...
function EmployeeCard({
  ...
  const joinDate = emp.createdAt
    ? new Date(emp.createdAt as string).toLocaleDateString()
    : "—";
...
export default function EmployeeManagement() {
  ...
  const [confirmAction, setConfirmAction] = useState<{ type: ConfirmActionType; emp: EmployeeRecord } | null>(null);
  ...
  const handleApprove = async (uid: string) => {
    ...
    const success = await updateEmployeeDetail(businessId as string, uid, { status: "active" });
    if (success) {
      setEmployees((prev) =>
  ...
  const handleDecline = async (uid: string) => {
    setActionLoading(uid);
    const success = await updateEmployeeDetail(businessId as string, uid, { status: "rejected" });
    if (success) {
      setEmployees((prev) =>
  ...
  const handleRemove = async (uid: string) => {
    setActionLoading(uid);
    try {
      await deleteDoc(doc(db, "businesses", businessId as string, "Employees", uid));
  ...
  const handleSaveStatus = async (uid: string, newStatus: string, newRole: string) => {
    ...
    setSavingEdit(true);
    try {
      const success = await updateEmployeeDetail(businessId as string, uid, { status: newStatus, role: newRole });
      if (success) {
        setEmployees((prev) =>
