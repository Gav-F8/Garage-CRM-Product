...
import { NHTSA, VEHICLE_TYPES, COLORS, YEARS } from "../lib/utils";
import { readMakesCache, writeMakesCache } from "../lib/cache";
...
function validate(form: VehicleForm): VehicleFormErrors {
...
export function CreateVehicleModal({ businessId, customers, onClose, onCreated }: CreateVehicleModalProps) {
  const [form, setForm] = useState<VehicleForm>(BLANK);
  const [errors, setErrors] = useState<VehicleFormErrors>({});
  const [saving, setSaving] = useState(false);
  const [makes, setMakes] = useState<string[]>([]);
  const [models, setModels] = useState<string[]>([]);
  const [vinMsg, setVinMsg] = useState<{ type: "ok" | "err"; text: string } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${NHTSA}/GetAllMakes?format=json`)
      .then((r) => r.json())
      .then((d) => {
        const sortedMakes: string[] = d.Results.map((m: { Make_Name: string }) => m.Make_Name).sort();
  ...
  const setField = (name: keyof VehicleForm, value: string) => {
  ...
  const decodeVin = async () => {
    ...
    try {
      ...
      const get = (v: string): string =>
        data.Results.find((r: { Variable: string }) => r.Variable === v)?.Value || "";
